...
class FakeTaskBitrix(SemanticBitrixFake):
    ...
    async def _dispatch(self, method: str, params: dict) -> Any:
        ...
        if self.missing:
            # Живой контракт портала (23.07): несуществующая задача — это
            # ПУСТОЙ result без ошибки, а не error-ответ.
            return []
...
async def test_deleted_task_cancels_ping(db, bot, session):
    """Удалённая задача (пустой result живого контракта) снимает пинг."""
    await db.add_reminder(CHAT, TEXT, DUE, "task", TASK)
...
async def test_plain_not_found_error_cancels_ping(db, bot, session):
    """Явная ошибка с чистым «задача не найдена» (без «недоступна») снимает пинг.

    Запас на конфигурации, где портал отвечает ошибкой, а не пустым result.
    """
    await db.add_reminder(CHAT, TEXT, DUE, "task", TASK)

    class PlainMissingBitrix(SemanticBitrixFake):
        async def _dispatch(self, method: str, params: dict) -> Any:
            raise ErrorInServerResponseException("0: Задача не найдена")

    await tasks.reconcile_task_reminders(PlainMissingBitrix(), db)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        # если рядом есть слова про «не найдена». Сам код с подчёркиваниями
        # тоже допускает «нет прав», какой бы текст ни шёл рядом.
        ...
        "ERROR_TASK_NOT_FOUND_OR_NOT_ACCESSIBLE: Task not found",
    ],
)
