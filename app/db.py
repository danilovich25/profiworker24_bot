...
class Database:
    ...
    async def update_task_reminder(
        ...
        со сроком в очереди обязаны совпадать с фактическими (и с ответом
        пользователю). Текст строит вызывающий по МОМЕНТУ, который реально
        останется в записи (due_ts или сохранённый срок при due_ts=None) —
        ...
        async with aiosqlite.connect(self.path) as conn:
            cur = await conn.execute(
                "SELECT id, due_ts FROM reminders "
                "WHERE kind = 'task' AND entity_id = ? AND status = ?",
                (task_id, REMINDER_PENDING),
            )
            row = await cur.fetchone()
            if row is None:
                return False, None
    ...
    async def mark_rearm_checked(self, reminder_id: int) -> None:
        ...
        Ставится после ЛЮБОЙ попытки скана, включая сбойную: не отмечать
        сбой значило бы держать сделку с постоянной ошибкой чтения в голове
        очереди вечно и не пускать к хвосту. Повтор сбойной — следующим
        кругом ротации.
        """
        async with aiosqlite.connect(self.path) as conn:
            await conn.execute(
